...
----- SEGUNDA TABLA (creada a partir de la primera) ---

SELECT *
INTO prediccion_fin_mundo
FROM eventos_apocalipticos;

SELECT * FROM eventos_apocalipticos

---- número eventos con letra D y A en cada decada ---

WITH consulta AS (
    SELECT
        FLOOR(DATEPART(YEAR, fecha_evento) / 10) * 10 AS Decada,
        nombre_evento
    FROM eventos_apocalipticos
)
SELECT
    Decada,
    SUM(CASE WHEN LOWER(LEFT(nombre_evento, 1)) = 'a' THEN 1 ELSE 0 END) AS Conteo_A,
    SUM(CASE WHEN LOWER(LEFT(nombre_evento, 1)) = 'd' THEN 1 ELSE 0 END) AS Conteo_D
FROM consulta
GROUP BY Decada


---- días promedio hasta los eventos de cada decada ---

WITH consulta AS (
    SELECT
        FLOOR(DATEPART(YEAR, fecha_evento) / 10) * 10 AS Decada,
        nombre_evento,
		DATEDIFF(day, GETDATE(),fecha_evento) as var_dias
    FROM eventos_apocalipticos
)
SELECT
    Decada,
	AVG(var_dias) dias_promedio
FROM consulta
GROUP BY 
	Decada
